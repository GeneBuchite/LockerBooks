...
If (isset($_POST['myDate'])) {
    ...
    echo $_COOKIE['bookDate'] . " =Bookdate" . $_COOKIE['bookDate'] . ' BookDate at By Cooklie';
    $dd = $_COOKIE['bookDate'];
...
if (isset($_POST['ConfirmDelete'])) {
    $mID = $_POST['DelID'];
    echo $mID . " Will Be Delected";
    include "conn.php";
    $dd = $_SESSION['bookDate'];
    // sql to delete a record
    $sql = "DELETE FROM Beef WHERE Counter=$mID";

    if ($conn->query($sql) === TRUE) {
        echo "Record deleted successfully";
        echo $dd . " = Stringdd";
        echo $_SESSION['bookDate'] . "= SessionBookDate";
        echo $_COOKIE['bookDate'] . " =BookDatye";
    } else {
...
if (isset($_POST['Delete'])) {
    $dd = $_SESSION['bookDate'];
    ?> <form action="" method="POST"> <h3> <?php
            echo "Do You Really want to delete " . $_POST['ID'] . " entry for " . $_POST['FullName'];
            ?>  </h3> <input type="submit" value="ConfirmDelete" name="ConfirmDelete" />
        <input  class="buttonS"  type="submit" value ="NoDelete" name="NoDelete" />
        <input  class="buttonG" type="text" value="<?php echo $_POST['ID']; ?>"  name="DelID" />
        <input type ="text" value="<?php echo $dd; ?>" name="myDate"/>

    </form>

    <?php
    echo $_POST['ID'] . " = ID  ";
    die('PLacde Delete Code Hdere');
}
include 'Conn.php';
//2017-04-28

$sql = "Select * FROM beef WHERE Type = '$q' Order By DeliveryDate DESC, `Tag Number` DESC LIMIT 1 ";
...
if (isset($_SESSION['Mydte'])) {
    $dd = $_SESSION['Mydte'];
    echo "MyDate Was Set";
} else {
    echo "MyDte Was Not Set";
}
echo $dd . " Before the sql statement";
            ...
            <div class="row">
                <div class="column">
                    <div class="flex-container">
                        <div> Live Weight </br>
                            <input type="text"  name="LiveWeight" id="lw" size="4" onblur="NextAnimal()"   autofocus ></div>
                        <div> Tag Number </br>
                            <input  type="text" name="TagNumber" onfocus="clearFields()" value='0' id="tn" size="4"></div>
                        <div> Order </br>
                            <input type="text" name="Order" id="o" size="4"></div>
                        <divl> Full Name </br>
                            <input class="form-control" list="fnms" name="FullName" id="fn" onblur="fillOwner()" >
                            <datalist id="fnms">
                                <?php
                                include 'Conn.php';
                        ...
                        <divl> Owner </br>
                            <input class="form-control" list="fnms" type="text" name="Owner" size="30" id ="ow"  />
                        </divl>
                        <div> Number Of Amimals </br>
                            <input type="text" name="Times" value="1" id="times" size="4" onblur ="document.getElementById('a').focus();" /> </div>
                    ...
                    <div class="flex-container">
                        ...
                        <div><input type="button"  name="Edit Weights" value="Edit Weights" onclick="EditWeights()"/> 

                        </div>
                        <div> Liver  Weights**: </br>
                            <input style="visibility: hidden" type="text" name="LiverWeight" id="lwt" value="0"  size="4"></div>
                        <div> Hanging Weight </br>
                            <input style="visibility: hidden" type="text" name="HangingWeight" id="hw" value="0" size="4">
                        </div>    
                        <div> Dressed Weight </br>
                            <input style="visibility: hidden"  type="text" name="DressedWeight" id="dw"  value="0" size="4">
                        </div>    
                        ...
                        <divl> Breed </br>
                            <input type="text" name="Breed" size="30" id ="br" onchange="document.getElementById('myNumOBeef').focus();" ></divl>
                    ...
                    <div class="column2">
                        <div class="flex-containers">
                            ...
                            <div> Number Of Beef 
                                <div> <input type="radio" name="NumberOfBeef" value="1" checked="checked" id="myNumOBeef" onchange="NOB(1)" /> Whole</div>
                                <div> <input type="radio" name="NumberOfBeef" value=".5" onchange="NOB(.5)" /> Half </div>
                                <div><input type="radio" name="NumberOfBeef" value=".33" onchange="NOB(.33)"/> Third </div>
                                <div><input type="radio" name="NumberOfBeef" value=".25" onchange="NOB(.25)"/> Quarters </div>
                                <div><input type="radio" name="NumberOfBeef" value=".51" onchange="NOB(.51)"/> One Half Two Quarters </div> 
                                <div><input type="radio" name="NumberOfBeef" value=".67" onchange="NOB(.67)"/> Two Thirds And One Third </div>
                                <div><input type="text" name="NumBeef" id="NumBeef" readonly Value="1" onfocus="document.getElementById('UD').focus();" /></div>
                            </div>
                        </div>
                        <div class="flex-containers">
                            ...
                            <div> Database 
                                <div>
                                    ...
                                    <div>  </div>
                                    <div><input class="buttonC" type="submit" value="Split Carcass" name="Split" id="SP" formaction="splitCarcass.php" formmethod="POST" />  </div>
                                    <div></div>
...
<script>
    function fillOwner() {
        var fnn = document.getElementById('ow').value;
        var fn = document.getElementById('fn').value;

        if (fnn.length === 0) {
//             console.log('It is Zero');
            document.getElementById('ow').value = fn;
        }
...
<script>
    function NOB(N) {
        ...
        document.getElementById("UD").focus();
        switch (N) {
            case 1:
                document.getElementById('times').value = 1;
                break;
            case .5:
                document.getElementById('times').value = 2;
                break;
            case .33:
                document.getElementById('times').value = 3;
                break;
            case .25:
                document.getElementById('times').value = 4;
                break;
            case .51:
                document.getElementById('times').value = 3;
                document.getElementById('NumBeef').value = 0.5;
                break;
            case .67:
                document.getElementById('times').value = 2;
                break;               
    ...
    function splitCarcass() {
        alert('Start Coding Here');
    }
</script>
...
<script>
    ...
    function setCuttingOrder() {
        ...
        document.getElementById('ow').focus();
    }
